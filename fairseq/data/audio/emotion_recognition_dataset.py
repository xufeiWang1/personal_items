...
@dataclass
class EmotionRecognitionDatasetItem(object):
    index: int
...
class EmotionRecognitionDataset(FairseqDataset):
    ...
    def __init__(
        ...
        self.cfg = cfg
        self.audio_paths, self.video_paths, self.n_frames  = audio_paths, video_paths, n_frames
        self.n_samples = len(audio_paths)
        assert len(audio_paths) == len(video_paths)
        assert len(n_frames) == self.n_samples > 0
        # assert src_texts is None or len(src_texts) == self.n_samples
        # assert tgt_texts is None or len(tgt_texts) == self.n_samples
        assert texts is None or len(texts) == self.n_samples
        assert speakers is None or len(speakers) == self.n_samples
        # assert src_langs is None or len(src_langs) == self.n_samples
        # assert tgt_langs is None or len(tgt_langs) == self.n_samples
        assert ids is None or len(ids) == self.n_samples
        assert (tgt_dict is None and texts is None) or (
            tgt_dict is not None and texts is not None
        )
        ...
        self.tgt_lens = self.get_text_lens_and_check_oov()
        self.append_eos = append_eos

        self.datatype = self.cfg.config.get("datatype", None)
        if self.datatype == "bin_offset":
            self.read_bin_file_no_shuffle = self.cfg.config.get("read_bin_file_no_shuffle", True)
        else:
            self.datatype = None
    ...
    def get_text_lens_and_check_oov(self):
        ...
        for i in range(self.n_samples):
            tokenized = self.get_tokenized_text(i).split(" ")
            oov_tokens = [
                t
                for t in tokenized
    ...
    def get_tokenized_text(self, index: int):
        text = self.tokenize(self.pre_tokenizer, self.texts[index])
    ...
    def _get_source_video(self, index: int) -> torch.Tensor:
    ...
    def __getitem__(self, index: int) -> EmotionRecognitionDatasetItem:
        # only applied for audio for "bin_offset" datatype
        if self.datatype == "bin_offset":
            if self.read_bin_file_no_shuffle is False:
            ...
            audio_data = self._get_source_audio(index)
            video_data = self._get_source_video(index)
            audio_data = self.pack_frames(audio_data)
            video_data = self.pack_frames(video_data)

        text = None
        if self.texts is not None:
            tokenized = self.get_tokenized_text(index)
            text = self.tgt_dict.encode_line(
        ...
        return EmotionRecognitionDatasetItem(
            index=index, audio_data=audio_data, video_data=video_data, text=text, target=target, speaker_id=speaker_id
        )
    ...
    def collater(
        self, samples: List[EmotionRecognitionDatasetItem], return_order: bool = False
    ) -> Dict:
        if len(samples) == 0:
            return {}
        ...
        video_frames = _collate_frames([x.video_data for x in samples])
        # sort samples by descending number of frames
        n_audio_frames = torch.tensor([x.audio_data.size(0) for x in samples], dtype=torch.long)
        n_audio_frames, order = n_audio_frames.sort(descending=True)
        n_video_frames = torch.tensor([x.video_data.size(0) for x in samples], dtype=torch.long)
        n_video_frames.index_select(0, order)
        indices = indices.index_select(0, order)
        targets = targets.index_select(0, order)
        audio_frames = audio_frames.index_select(0, order)
        video_frames = video_frames.index_select(0, order)
        # frames = frames.index_select(0, order)

        # target, target_lengths = None, None
        texts, texts_lengths = None, None
        # prev_output_tokens = None
        ntokens = None
        if self.texts is not None:
            texts = fairseq_data_utils.collate_tokens(
            ...
            texts = texts.index_select(0, order)
            # target = target.index_select(0, order)
            texts_lengths = torch.tensor(
        ...
        net_input = {
            "src_audio_tokens": audio_frames,
            "src_video_tokens": video_frames,
            "src_text_tokens": texts,
            "src_audio_lengths": n_audio_frames,
            "src_video_lengths": n_video_frames,
            "src_text_lengths": texts_lengths
            # "src_tokens": frames,
            # "src_lengths": n_frames,
            # "prev_output_tokens": prev_output_tokens,
        }

        # path_list = [self.audio_paths[i] for i in indices.tolist() ]
        out = {
            "id": indices,
            # "path": path_list,
            "net_input": net_input,
    ...
    def ordered_indices(self):
        # keep the original order for bin_offset data
        if self.datatype == "bin_offset" and (self.read_bin_file_no_shuffle is True):
...
class EmotionRecognitionDatasetCreator(object):
    # mandatory columns
    ...
    def _from_list(
        ...
        video_root = Path(cfg.audio_root)
        ids = [s[cls.KEY_ID] for s in samples]
        audio_paths = [(audio_root / s[cls.KEY_AUDIO]).as_posix() for s in samples]
        video_paths = [(video_root / s[cls.KEY_VIDEO]).as_posix() for s in samples]
    ...
    @classmethod
    def get_size_ratios(
        cls, datasets: List[EmotionRecognitionDataset], alpha: float = 1.0
    ) -> List[float]:
