...
from fairseq.tasks.fbank_pretraining import FBankPretrainingConfig, FBankPretrainingTask
...
@dataclass
class FBankFinetuningConfig(FBankPretrainingConfig):
...
@register_task("fbank_finetuning", dataclass=FBankFinetuningConfig)
class FBankFinetuningTask(FBankPretrainingTask):
    """ """

    cfg: FBankFinetuningConfig
    ...
    def load_dataset(
        self, split: str, epoch: int=1, task_cfg: FBankFinetuningConfig = None, **kwargs
    ):
        super().load_dataset(split, epoch, task_cfg, **kwargs)
