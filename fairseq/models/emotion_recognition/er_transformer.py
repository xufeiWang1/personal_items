...
@dataclass
class ERTransformerConfig(FairseqDataclass):
...
@register_model("er_transformer", dataclass=ERTransformerConfig)
class ERTransformerModel(BaseFairseqModel):
    ...
    def forward(self, src_audio_tokens, src_video_tokens, src_text_tokens, src_audio_lengths, src_video_lengths, src_text_lengths):
        """
        The forward method inherited from the base class has a **kwargs
        argument in its input, which is not supported in torchscript. This
        method overwrites the forward method definition without **kwargs.
        """
        print (f"src_audio_tokens: {src_audio_tokens.size()}")
        print (f"src_video_tokens: {src_video_tokens.size()}")
        print (f"src_text_tokens: {src_text_tokens.size()}")
        print (f"src_audio_lengths: {src_audio_lengths}")
        print (f"src_video_lengths: {src_video_lengths}")
        print (f"src_text_lengths: {src_text_lengths}")
        print ("xiexie0")
        # acoustic encoder
        encoder_outs = self.encoder(src_tokens=src_audio_tokens, src_lengths=src_audio_lengths)
