...
def extract_fbank_features(
    ...
    _waveform, _ = convert_waveform(waveform, sample_rate, to_mono=True) # 相当于没干啥 调用了/home/xufei/fairseq-asr/fairseq/data/audio/audio_utils.py里的函数
    # Kaldi compliance: 16-bit signed integers
    _waveform = _waveform * (2 ** 15) # 将原本一句话的每帧波形值乘32768
    _waveform = _waveform.numpy() # ？ tensor转成numpy数组

    features = _get_kaldi_fbank(_waveform, sample_rate, n_mel_bins) # 提取fbank特征，但不成功，该程序预料到不成功并又提供了另一个提取fbank的方式 调用了/home/xufei/fairseq-asr/fairseq/data/audio/audio_utils.py里的函数
    if features is None:
    ...
    if output_path is not None: # 注意！！！ 该操作将内容存进了npy文件
        np.save(output_path.as_posix(), features) # 将提取出的一个segment 的fbank特征存储到 /home/xufei/gigaspeech-dev/data4fairseq/dev-tidy_xufei-fbank80/POD1000000004_0.npy 里
    return features
