...
class Data2VecFBankModel(BaseFairseqModel):
    def __init__(self, cfg: Data2VecAudioConfig):
        ...
        self.linear = torch.nn.Linear(cfg.fbank_encoder_dim, cfg.fbank_encoder_dim)
        nn.init.xavier_normal_(self.linear.weight)
        nn.init.zeros_(self.linear.bias)
        self.dropout = torch.nn.Dropout(cfg.dropout_features)
        self.use_fbank_feature = True
    ...
    def extract_fbank_features(self, source, src_lengths=None):
        ...
        with torch.no_grad():
            for batch_idx in range(source.size(0)):
                ...
                features = features.type(data_dtype)
                ...
                if self.encoder.training is True and False:
    ...
    def forward(
        ...
        if self.use_fbank_feature is True:
            ...
            features, _ = self.extract_fbank_features(source)
            features = features.transpose(0, 1)
